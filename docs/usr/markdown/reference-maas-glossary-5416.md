...
- Module finalisation - stage four is the end of the boot process, installing packages and executing any user-supplied configuration scripts.
...
Edge clouds can be planned and enhanced by using NUMA and SR-IOV techniques.  NUMA can help you create SMP nodes on VM cores the shortest (network) distance away from your application.  SR-IOV can, in general, reduce network latency even more by eliminating core involvement in network traffic.  You can create virtual machines and assign NUMA nodes to minimise network latency, and then ensure that   MAAS gives you NUMA tools to find out whether you're achieving this sort of optimization, and help you make decisions about how to adjust and improve over time.
...
Besides topology, networks can also be classified by their size, range, or "reach."  One such classification is the [Local Area Network (LAN)](https://en.wikipedia.org/wiki/Local_area_network#firstHeading), which connects computers in close (about 300 feet).
...
To add this PPA, then, hit the 'Add repository' button and fill in the fields.  Before saving, confirm that:

- The repository type is set to **PPA**.
- The **Location** field contains `ppa:canonical-support/support-tools`.
- The **Key** field references the fingerprint `9360754F`.
- The repository is marked as **Enabled**.
...
MAAS comes equipped with the official Ubuntu repository `archive.ubuntu.com` as well as the equivalent for architectures other than i386 and amd64: `ports.ubuntu.com`.  Both entries appear in the default configuration table with the primary components (`main`, `restricted`, `universe`, and `multiverse`) enabled.
...
Before saving, make sure that the 'Add repository' form reflects the collected details; for example:

- **Type**: `deb`
- **Name**: A meaningful label, such as `Google Chrome`
