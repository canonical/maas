...
(or ``keyring_data``) for validating index and image signatures from that location.
...
The following example use the MAAS CLI to list the boot sources and the boot source
selections for a particular cluster.  Assuming that ``CLUSTER_UUID`` is the UUID of the cluster
being examined and the CLI ``PROFILE`` is the name of the profile being used::
...
Let's say you want to restrict the images being downloaded to only one architecture and 
one release; starting from the configuration described above, you would need to:
...
- Update the architecture list of the "Trusty" selection so that only the image for amd64
  will be downloaded (this is the selection '2' of the source '1')::
...
Let's say you want to import the images from a different location: you would need to
to change the source's url and keyring::
