...
- Use resource pools when access control is (or might later become) the priority. Pools are the scope for MAAS's built-in fine-grained authorization: you can grant a user group per-pool entitlements so that, for example, "team A can only deploy machines in pool X." Even without entitlements they work as a single-label grouping mechanism to separate environments such as development, staging, and production. See [User groups and entitlements](/how-to-guides/user-groups-and-entitlements.md).

### Machine grouping cheat sheet

| Grouping tool        | General usage in MAAS                                                                 | Special case: Juju                                                               | Special case: OpenStack                                                              | Special case: Access control                                              |
|----|----|----|----|----|
| **Tags**             | Flexible labels, multiple per machine. Use for filtering, commissioning, or deployment selection. Can be manual or automatic (XPath). | Juju ignores tags directly, but you can target machines indirectly by tag when adding them to models or constraints. | Same as Juju—tags aren’t first-class in OpenStack, but can help when mapping hardware to specific use cases before handoff. | No tie-in to authorization; tags are just metadata.                       |
| **Availability zones (AZs)** | One per machine. Conceptually “fault domains.” In plain MAAS, it’s just a label.                         | Juju respects AZs as failover zones. Workloads can be scheduled across zones for HA. | OpenStack also respects AZs as placement/failover domains; ensures workloads don’t all land in the same failure point. | No authorization connection—zones don’t control access, just placement.   |
| **Resource pools**   | One per machine. Also the scope for MAAS's built-in fine-grained authorization.        | Juju does not use pools.                                                            | OpenStack does not use pools.                                                        | Pools are the unit of access control. Per-pool entitlements restrict which user groups can view, deploy, or edit those machines. |
