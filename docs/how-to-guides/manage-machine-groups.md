...
- Use resource pools when organizational boundaries are the priority. Pools are a single-label grouping mechanism that can separate environments such as development, staging, and production.

### Machine grouping cheat sheet

| Grouping tool        | General usage in MAAS                                                                 | Special case: Juju                                                               | Special case: OpenStack                                                              |
|----|----|----|----|
...
| **Availability zones (AZs)** | One per machine. Conceptually “fault domains.” In plain MAAS, it’s just a label.                         | Juju respects AZs as failover zones. Workloads can be scheduled across zones for HA. | OpenStack also respects AZs as placement/failover domains; ensures workloads don’t all land in the same failure point. |
| **Resource pools**   | One per machine. Just a label in plain MAAS, often used for org or project grouping.   | Juju does not use pools.                                                            | OpenStack does not use pools.                                                        |
