...
Understanding what MAAS is doing under the hood is critical when troubleshooting unexpected behavior — whether a machine fails to deploy, a controller won’t respond, or an API call returns errors.  MAAS uses `systemd` journal logs to provide an efficient and centralized way to inspect logs, in context, in real time.
