...
(across interfaces or configurations) and as duplicates across different
machines, which breaks the assumption that one hardware MAC maps to one unique
MAAS identity. Only *physical* interfaces are affected; bonds, bridges, and
VLAN interfaces legitimately reuse the MAC address of one of their child
interfaces and are never reported.
