...
"""HTTP server for the OpenBMC power driver, listening on a UNIX domain socket."""
...
from maas_power_driver_openbmc.driver import OpenBMCPowerDriver
...
def main():
    """Start the OpenBMC power driver HTTP server."""
    parser = argparse.ArgumentParser(description="OpenBMC Power Driver Service")
    parser.add_argument(
        "command",
    ...
    parser.add_argument(
        "--socket-path",
        default="/var/snap/maas-power-driver-openbmc/common/power-drivers/openbmc.sock",
    ...
    PowerDriverHandler.driver = OpenBMCPowerDriver()
    ...
    logger.info("OpenBMC power driver listening on %s", args.socket_path)
