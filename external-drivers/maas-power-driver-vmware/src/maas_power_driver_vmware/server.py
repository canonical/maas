...
"""HTTP server for the VMware power driver, listening on a UNIX domain socket."""
...
from maas_power_driver_vmware.driver import VMwarePowerDriver
...
def main():
    """Start the VMware power driver HTTP server."""
    parser = argparse.ArgumentParser(description="VMware Power Driver Service")
    parser.add_argument(
        "command",
    ...
    PowerDriverHandler.driver = VMwarePowerDriver()
    ...
    logger.info("VMware power driver listening on %s", args.socket_path)
