...
"""HTTP server for the IPMI power driver, listening on a UNIX domain socket."""
...
from maas_power_driver_ipmi.driver import IPMIPowerDriver
...
def main():
    """Start the IPMI power driver HTTP server."""
    parser = argparse.ArgumentParser(description="IPMI Power Driver Service")
    parser.add_argument(
        "command",
    ...
    parser.add_argument(
        "--socket-path",
        default="/var/snap/maas-power-driver-ipmi/common/power-drivers/ipmi.sock",
    ...
    PowerDriverHandler.driver = IPMIPowerDriver()
    ...
    logger.info("IPMI power driver listening on %s", args.socket_path)
