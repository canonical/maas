...
"""HTTP server for the APC power driver, listening on a UNIX domain socket."""
...
from maas_power_driver_apc.driver import APCPowerDriver
...
def main():
    """Start the APC power driver HTTP server."""
    parser = argparse.ArgumentParser(description="APC Power Driver Service")
    parser.add_argument(
        "command",
    ...
    parser.add_argument(
        "--socket-path",
        default="/var/snap/maas-power-driver-apc/common/power-drivers/apc.sock",
    ...
    PowerDriverHandler.driver = APCPowerDriver()
    ...
    logger.info("APC power driver listening on %s", args.socket_path)
