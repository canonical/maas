...
"""RECS power driver implementation."""
...
class RECSPowerDriver:
    """RECS power driver using wget to communicate with RECS BMCs."""

    def _run_wget(self, context: dict, action: str) -> str:
        """Run a wget command to the RECS BMC."""
        ...
        url = f"{power_address.rstrip('/')}/cgi-bin/power.cgi?action={action}"
    ...
    def query(self, system_id: str, context: dict) -> str:
        ...
        try:
            ...
            logger.error("RECS query failed: %s", e)
            raise
    ...
    def set_boot_order(self, system_id: str, context: dict) -> None:
        logger.warning("set_boot_order is not supported by the RECS driver")
