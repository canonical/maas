...
"""HTTP server for the RECS power driver, listening on a UNIX domain socket."""
...
from maas_power_driver_recs.driver import RECSPowerDriver
...
def main():
    """Start the RECS power driver HTTP server."""
    parser = argparse.ArgumentParser(description="RECS Power Driver Service")
    parser.add_argument(
        "command",
    ...
    parser.add_argument(
        "--socket-path",
        default="/var/snap/maas-power-driver-recs/common/power-drivers/recs.sock",
    ...
    PowerDriverHandler.driver = RECSPowerDriver()
    ...
    logger.info("RECS power driver listening on %s", args.socket_path)
