...
"""HTTP server for the Raritan power driver, listening on a UNIX domain socket."""
...
from maas_power_driver_raritan.driver import RaritanPowerDriver
...
def main():
    """Start the Raritan power driver HTTP server."""
    parser = argparse.ArgumentParser(description="Raritan Power Driver Service")
    parser.add_argument(
        "command",
    ...
    parser.add_argument(
        "--socket-path",
        default="/var/snap/maas-power-driver-raritan/common/power-drivers/raritan.sock",
    ...
    PowerDriverHandler.driver = RaritanPowerDriver()
    ...
    logger.info("Raritan power driver listening on %s", args.socket_path)
