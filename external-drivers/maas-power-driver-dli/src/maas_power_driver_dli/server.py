...
"""HTTP server for the DLI power driver, listening on a UNIX domain socket."""
...
from maas_power_driver_dli.driver import DLIPowerDriver
...
def main():
    """Start the DLI power driver HTTP server."""
    parser = argparse.ArgumentParser(description="DLI Power Driver Service")
    parser.add_argument(
        "command",
    ...
    parser.add_argument(
        "--socket-path",
        default="/var/snap/maas-power-driver-dli/common/power-drivers/dli.sock",
    ...
    PowerDriverHandler.driver = DLIPowerDriver()
    ...
    logger.info("DLI power driver listening on %s", args.socket_path)
