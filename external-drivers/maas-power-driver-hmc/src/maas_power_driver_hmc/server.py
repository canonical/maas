...
"""HTTP server for the IBM HMC power driver, listening on a UNIX domain socket."""
...
from maas_power_driver_hmc.driver import HMCPowerDriver
...
def main():
    """Start the IBM HMC power driver HTTP server."""
    parser = argparse.ArgumentParser(description="IBM HMC Power Driver Service")
    parser.add_argument(
        "command",
    ...
    PowerDriverHandler.driver = HMCPowerDriver()
    ...
    logger.info("IBM HMC power driver listening on %s", args.socket_path)
