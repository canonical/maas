...
"""IBM HMC power driver implementation."""
...
class HMCPowerDriver:
    """IBM HMC power driver using zhmcclient."""

    def _get_partition(self, context: dict):
        """Get the HMC partition object."""
        try:
        ...
        try:
            ...
            try:
                ...
                state_map = {
                    ...
                    "deactivating": "off",
                    "powering-off": "off",
                    "waiting-for-virtualization-mode": "off",
                }
                return state_map.get(state, "unknown")
            finally:
                hmc.logout()
        except Exception as e:
            logger.error("HMC query failed: %s", e)
            raise
    ...
    def set_boot_order(self, system_id: str, context: dict) -> None:
        """Set boot order (not supported by HMC driver)."""
        logger.warning("set_boot_order is not supported by the HMC driver")
