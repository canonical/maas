...
"""HTTP server for the Intel AMT power driver, listening on a UNIX domain socket."""
...
from maas_power_driver_amt.driver import AMTPowerDriver
...
def main():
    """Start the Intel AMT power driver HTTP server."""
    parser = argparse.ArgumentParser(description="Intel AMT Power Driver Service")
    parser.add_argument(
        "command",
    ...
    parser.add_argument(
        "--socket-path",
        default="/var/snap/maas-power-driver-amt/common/power-drivers/amt.sock",
    ...
    PowerDriverHandler.driver = AMTPowerDriver()
    ...
    logger.info("Intel AMT power driver listening on %s", args.socket_path)
