...
"""HTTP server for the Cisco UCSM power driver, listening on a UNIX domain socket."""
...
from maas_power_driver_ucsm.driver import UCSMPowerDriver
...
def main():
    """Start the Cisco UCSM power driver HTTP server."""
    parser = argparse.ArgumentParser(description="Cisco UCSM Power Driver Service")
    parser.add_argument(
        "command",
    ...
    parser.add_argument(
        "--socket-path",
        default="/var/snap/maas-power-driver-ucsm/common/power-drivers/ucsm.sock",
    ...
    PowerDriverHandler.driver = UCSMPowerDriver()
    ...
    logger.info("Cisco UCSM power driver listening on %s", args.socket_path)
