...
"""HTTP server for the Eaton power driver, listening on a UNIX domain socket."""
...
from maas_power_driver_eaton.driver import EatonPowerDriver
...
def main():
    """Start the Eaton power driver HTTP server."""
    parser = argparse.ArgumentParser(description="Eaton Power Driver Service")
    parser.add_argument(
        "command",
    ...
    parser.add_argument(
        "--socket-path",
        default="/var/snap/maas-power-driver-eaton/common/power-drivers/eaton.sock",
    ...
    PowerDriverHandler.driver = EatonPowerDriver()
    ...
    logger.info("Eaton power driver listening on %s", args.socket_path)
