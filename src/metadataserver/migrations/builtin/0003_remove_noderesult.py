...
def noderesult_to_scriptresult(apps, schema_editor):
    ...
    for node_result in NodeResult.objects.all():
        ...
        if (node_result.name.endswith('.out') or
                node_result.name.endswith('.err')):
        ...
        script_result, _ = ScriptResult.objects.get_or_create(
            created=node_result.created, updated=node_result.updated,
            script_set=script_set, status=status,
            exit_status=node_result.script_result, script_name=script_name)
