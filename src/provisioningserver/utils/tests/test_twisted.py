...
from provisioningserver.utils.twisted import (
    ...
    IAsynchronous,
    ISynchronous,
    LegacyLogObserverWrapperForMAAS,
    LONGTIME,
...
class TestTerminateProcess(MAASTestCase):
    ...
    def setUp(self):
        ...
        self.patch(
            ...
            Mock(wraps=twisted_module._os_kill))
    ...
    def terminateSignalPrinter(self, process, protocol):
        ...
        terminateProcess(
            process.pid, protocol.done, quit_after=0.1,
            kill_after=0.2)
...
class TestLegacyLogObserverWrapperForMAAS(MAASTestCase):
    """Scenario tests for `LegacyLogObserverWrapperForMAAS`."""
    ...
    def processEvent(self, event):
        events = []
        observer = LegacyLogObserverWrapperForMAAS(events.append)
        observer(event)
...
class TestLegacyLogObserverWrapperForMAAS_Installation(MAASTestCase):
    """Tests for `LegacyLogObserverWrapperForMAAS`."""
    ...
    def test__installs_wrapper_to_log_module(self):
        with patch.object(log, "LegacyLogObserverWrapper", sentinel.unchanged):
            ...
            self.assertThat(
                log.LegacyLogObserverWrapper,
                Is(LegacyLogObserverWrapperForMAAS))
    ...
    def test__rewraps_existing_observers(self):
        ...
        with patch.object(log, "LegacyLogObserverWrapper", OldWrapper):
            ...
            self.assertThat(
                log.theLogPublisher._legacyObservers,
                AllMatch(IsInstance(LegacyLogObserverWrapperForMAAS)))
            # They're registered with the new global log publisher too.
            self.assertThat(
                logger.globalLogPublisher._observers,
                ContainsAll(log.theLogPublisher._legacyObservers))
