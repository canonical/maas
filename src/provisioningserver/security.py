...
# Cache the AES-GCM key to avoid re-running PBKDF2 on every call.
# Cleared automatically whenever MAAS_SECRET changes (rotation or revocation).
_aesgcm_key: bytes | None = None
_aesgcm_lock = Lock()


def _clear_aesgcm_key() -> None:
    """Invalidate the cached AES-GCM key.

    Called by MAAS_SECRET.on_change so that a rotated or revoked secret is
    never used to decrypt traffic encrypted under the new key.
    """
    with _aesgcm_lock:
        global _aesgcm_key
        _aesgcm_key = None
...
MAAS_SECRET.on_change(_clear_aesgcm_key)
...
def fernet_encrypt_psk(message, raw=False):
    """Encrypt *message* with AES-256-GCM using the MAAS shared secret.

    Named ``fernet_*`` for import compatibility with callers that pre-date the
    Fernet→AES-GCM migration.
