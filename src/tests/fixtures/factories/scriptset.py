...
async def create_test_scriptset_entry(
    ...
    **extra_details: Any,
    ...
    The script set will contain a reference to a collection of scripts run
    against a node during its life cycle. For example, the script set run
    during commissioning.

    In order to create a script set entry, the following parameters should be
    defined:
    - node_id: ID of the node were the script set was run. That is the primary
      key in the node table.
