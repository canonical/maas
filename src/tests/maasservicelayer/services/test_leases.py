...
class TestIntegrationLeasesService:
    ...
    async def test_create_ignores_none_hostname(
        ...
        unknown_interface = (
            await services.interfaces.get_interfaces_for_mac(mac_address)
        )[0]
        ...
        dnsresources = await services.dnsresources.list(token=None, size=10)
        assert len(dnsresources.items) == 0
    ...
    async def test_creates_lease_for_physical_interface(
        ...
        ifaces = await services.interfaces.get_interfaces_for_mac(mac_address)
        discovered_ips = await services.staticipaddress.get_discovered_ips_in_family_for_interfaces(
            interfaces=ifaces, family=IpAddressFamily.IPV4
        )
        assert (
            len(discovered_ips) == 1
