...
from maascommon.utils import jwt as jwt_utils
...
def msm_enrol_mock(mocker, services: ServiceCollectionV3):
    ...
    services.msm.get_status = Mock(return_value=None)
    yield enrol_mock
...
class TestMSM:
    ...
    def test_enrol_no(self, mocker, msm_enrol_mock):
        mock_prompt = mocker.patch.object(
            msm, "prompt_yes_no", return_value=False
        )
        mocker.patch.object(
            jwt_utils, "decode_unverified_jwt", return_value=SAMPLE_JWT_PAYLOAD
        )
        opts = self._configure_kwargs()
        msm.Command().handle(**opts)
        mock_prompt.assert_called_once()
        msm_enrol_mock.assert_not_called()

    def test_enrol_yes(self, mocker, msm_enrol_mock):
        mock_prompt = mocker.patch.object(
        ...
        msm.Command().handle(**opts)
        mock_prompt.assert_called_once()
        msm_enrol_mock.assert_called_once_with(
            opts["enrolment_token"], metainfo=YAML_CONFIG
        )

    def test_enrol_non_interactive(self, mocker, msm_enrol_mock):
        mock_prompt = mocker.patch.object(
            msm, "prompt_yes_no", return_value=True
        )
        mocker.patch.object(
    ...
    def test_status_not_enroled(
        self, mocker, capfd, services: ServiceCollectionV3
    ):
        ...
        wf_exec.start_time = expected_started
        services.msm.temporal_service.query_workflow = AsyncMock(
            return_value=(True, wf_exec)  # is_pending
        )
