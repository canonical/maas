...
class TestGetFipsStatus:
    def test_file_missing_returns_not_enabled(self, tmp_path, monkeypatch):
        monkeypatch.setattr(
            "maascommon.fips.FIPS_PROC_PATH", tmp_path / "fips_enabled"
        )
        status = get_fips_status()
        assert status.enabled is False
        assert status.detection_source == "file_missing"
    ...
    def test_file_zero_returns_not_enabled(self, tmp_path, monkeypatch):
        ...
        status = get_fips_status()
        assert status.enabled is False
        assert status.detection_source == str(fips_file)
        assert status.detection_error is None
    ...
    def test_cache_info_shows_hit_after_second_call(
        self, tmp_path, monkeypatch
    ):
        fips_file = tmp_path / "fips_enabled"
        fips_file.write_text("0")
        ...
        get_fips_status()
        get_fips_status()
        info = get_fips_status.cache_info()
        assert info.hits >= 1
    ...
    def test_logs_warning_on_oserror(self, tmp_path, monkeypatch):
        fips_file = tmp_path / "fips_enabled"
        fips_file.write_text("1")
        monkeypatch.setattr("maascommon.fips.FIPS_PROC_PATH", fips_file)
    ...
    def test_returns_fips_status_dataclass(self, tmp_path, monkeypatch):
        fips_file = tmp_path / "fips_enabled"
        fips_file.write_text("0")
        monkeypatch.setattr("maascommon.fips.FIPS_PROC_PATH", fips_file)
        status = get_fips_status()
...
class TestIsFipsEnabled:
    def test_returns_false_when_not_fips(self, tmp_path, monkeypatch):
        monkeypatch.setattr(
            "maascommon.fips.FIPS_PROC_PATH", tmp_path / "missing"
        )
        assert is_fips_enabled() is False

    def test_returns_true_when_fips(self, tmp_path, monkeypatch):
        fips_file = tmp_path / "fips_enabled"
        fips_file.write_text("1")
        monkeypatch.setattr("maascommon.fips.FIPS_PROC_PATH", fips_file)
        assert is_fips_enabled() is True


class TestFIPSSSHConfig:
    def test_default_ciphers_are_fips_approved(self):
        for cipher in FIPS_SSH_CONFIG.ciphers:
            assert "aes" in cipher

    def test_default_kex_are_fips_approved(self):
        for kex in FIPS_SSH_CONFIG.kex:
            # All approved KEX use ECDH P-curves or DH with SHA-256
            assert any(token in kex for token in ("nistp", "group14-sha256"))

    def test_default_macs_are_hmac_sha2(self):
        for mac in FIPS_SSH_CONFIG.macs:
            assert "sha2" in mac

    def test_default_key_types_exclude_pure_dsa(self):
        # Pure DSA (ssh-dss) is FIPS-prohibited; ECDSA is allowed.
        for key_type in FIPS_SSH_CONFIG.key_types:
            assert "ssh-dss" not in key_type
            assert "ed25519" not in key_type

    def test_is_frozen(self):
        with pytest.raises((AttributeError, TypeError)):
            FIPS_SSH_CONFIG.ciphers = ("bad",)  # type: ignore[misc]

    def test_singleton_is_fips_ssh_config_instance(self):
        assert isinstance(FIPS_SSH_CONFIG, FIPSSSHConfig)
