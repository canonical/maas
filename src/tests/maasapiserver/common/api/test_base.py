...
class TestAPI:
    async def test_register_handlers(self) -> None:
        ...
        api = API(prefix="/v2", handlers=[Handler1(), Handler2()])
        app = FastAPI()
        api.register(app.router)
        routes = [(route.path, route.methods) for route in app.router.routes]
        assert ("/v2/handler1", {"GET"}) in routes
        assert ("/v2/handler1/foo", {"GET", "DELETE"}) in routes
        assert ("/v2/handler2/bar", {"GET"}) in routes
        assert ("/v2/handler2/baz", {"GET", "POST"}) in routes
