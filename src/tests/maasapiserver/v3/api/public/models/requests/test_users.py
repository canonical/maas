...
class TestPasswordComplexityEnforcement:
    ...
    def test_weak_password_rejected_when_hardening_active(self, build) -> None:
        ...
        assert any(
            e["loc"][-1] in ("password", "new_password")
    ...
    def test_strong_password_accepted_when_hardening_active(
        ...
        password = getattr(req, "new_password", None) or getattr(
            req, "password", None
        )
        assert password == _STRONG_PASSWORD
    ...
    def test_update_none_password_bypasses_complexity_check(self) -> None:
        # None is the "no-change" sentinel for optional password fields; the
        # validator short-circuits before reaching _enforce_password_complexity.
        _hardening._hardening_active = True
        req = UserUpdateRequestSelf(**_base_fields(new_password=None))
