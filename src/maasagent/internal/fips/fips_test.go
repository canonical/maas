...
// resetOnce replaces the package-level Once so IsEnabled(logger.Noop()) can be retested.
func resetOnce() {
	once = sync.Once{}
	enabled = false
}
...
func TestDetect_FileMissing(t *testing.T) {
	...
	result := detect(logger.Noop(), path)
...
func TestIsEnabled_CachesResult(t *testing.T) {
	...
	first := IsEnabled(logger.Noop())
	second := IsEnabled(logger.Noop())
...
func TestIsEnabled_NonFIPS(t *testing.T) {
	...
	assert.False(t, IsEnabled(logger.Noop()))
}
