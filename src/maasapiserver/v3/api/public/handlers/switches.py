...
from maasapiserver.v3.api.public.models.requests.switches import SwitchRequest
...
class SwitchesHandler(Handler):
    ...
    async def create_switch(
        ...
        existing_switches = await services.switches.list(
            page=1,
            size=10,
            query=QuerySpec(
                where=SwitchClauseFactory.with_mac_address(
                    switch_request.mac_address
                )
            ),
        )
        if existing_switches.total > 0:
        ...
        switch = await services.switches.create(switch_request.to_builder())
    ...
    async def update_switch(
        ...
        if not existing_switch:
            ...
            # Check for duplicate MAC address
        existing_switches_with_mac_address = await services.switches.list(
            page=1,
            size=10,
            query=QuerySpec(
                where=SwitchClauseFactory.with_mac_address(
        ...
        if existing_switches_with_mac_address.total > 0:
            raise AlreadyExistsException(
                details=[
                    BaseExceptionDetail(
                        type="SwitchAlreadyExists",
        ...
        switch = await services.switches.update_by_id(
            switch_id, switch_request.to_builder()
        )
