...
def register(mcp: FastMCP, pool: MAASClientPool) -> None:
    ...
    async def list_fabrics(page: int = 1, page_size: int = 100) -> str:
    ...
    async def get_fabric(fabric_id: int) -> str:
        async def _operation(client: MAASClient) -> str:
            response = await client.get(
                _FABRIC_PATH,
    ...
    async def get_vlan(fabric_id: int, vlan_id: int) -> str:
        async def _operation(client: MAASClient) -> str:
            response = await client.get(
                _VLAN_PATH,
    ...
    async def delete_fabric(fabric_id: int) -> str:
        async def _operation(client: MAASClient) -> str:
    ...
    async def delete_vlan(fabric_id: int, vlan_id: int) -> str:
        async def _operation(client: MAASClient) -> str:
            try:
                get_response = await client.get(
                    _VLAN_PATH,
