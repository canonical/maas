...
class Factory(maastesting.factory.Factory):
    ...
    def make_Switch(
            self, node=None, owner=None, **kwargs):
        if owner is None:
            owner = get_worker_user()
        nos_kwargs = {arg: kwargs.pop(arg)
    ...
    def make_Subnet(self, name=None, vlan=None, cidr=None, gateway_ip=RANDOM,
                    ...
                    allow_dns=True, allow_proxy=True, managed=True,
                    space=RANDOM_OR_NONE, description=None):
        if name is None:
        ...
        subnet = Subnet(
            ...
            dns_servers=dns_servers, rdns_mode=rdns_mode, allow_dns=allow_dns,
            allow_proxy=allow_proxy, managed=managed, description='')
        subnet.save()
    ...
    def make_ISCSIBlockDevice(
        ...
        if target is None:
            ...
            target = '%s:%s:%s:%s@%s:%s:%s:%s:%s' % (
                user, password, init_user, init_pass,
