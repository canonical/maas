...
class TestNodeAPI(APITestCase):
    ...
    def test_POST_start_sets_default_hwe_kernel(self):
        self.patch(
            BootResource.objects,
            'get_usable_kernels').return_value = ('hwe-t', 'hwe-u')
        osystem = make_usable_osystem(self, 'ubuntu', ['trusty'])
        distro_series = osystem['default_release']
        node = factory.make_Node(
        ...
        self.assertEqual(httplib.OK, response.status_code)
        self.assertEqual(
            'hwe-' + distro_series[0], reload_object(node).hwe_kernel)

    def test_POST_start_sets_hwe_kernel(self):
        self.patch(
            BootResource.objects,
            'get_usable_kernels').return_value = ('hwe-t', 'hwe-v')
        node = factory.make_Node(
            owner=self.logged_in_user, mac=True, power_type='ether_wake',
            architecture=make_usable_architecture(self))
        response = self.client.post(
    ...
    def test_POST_start_fails_with_nongeneric_arch_and_hwe_kernel(self):
        osystem = make_usable_osystem(self, 'ubuntu', ['trusty'])
        distro_series = osystem['default_release']
        node = factory.make_Node(
            owner=self.logged_in_user, mac=True, power_type='ether_wake',
    ...
    def test_POST_start_fails_with_missing_hwe_kernel(self):
        self.patch(
            BootResource.objects,
            'get_usable_kernels').return_value = ('hwe-t', 'hwe-u')
        osystem = make_usable_osystem(self, 'ubuntu', ['trusty'])
        ...
        self.assertEqual(
            {'hwe_kernel':
             ["hwe-v is not avaliable for %s/%s on %s." %
              (osystem['name'], distro_series, node.architecture)]},
            json.loads(response.content))

    def test_POST_start_fails_with_old_kernel_and_newer_release(self):
        self.patch(
            BootResource.objects,
            'get_usable_kernels').return_value = ('hwe-t', 'hwe-v')
        osystem = make_usable_osystem(self, 'ubuntu', ['vivid'])
        distro_series = osystem['default_release']
        node = factory.make_Node(
            owner=self.logged_in_user, mac=True, power_type='ether_wake',
            architecture=make_usable_architecture(
    ...
    def test_POST_start_fails_with_old_kernel_and_newer_min_hwe_kernel(self):
        self.patch(
            BootResource.objects,
            'get_usable_kernels').return_value = ('hwe-t', 'hwe-v')
        osystem = make_usable_osystem(self, 'ubuntu', ['precise'])
        distro_series = osystem['default_release']
        node = factory.make_Node(
            owner=self.logged_in_user, mac=True, power_type='ether_wake',
            architecture=make_usable_architecture(
    ...
    def test_POST_start_fails_with_no_avalible_kernels(self):
        self.patch(
            BootResource.objects,
            'get_usable_kernels').return_value = ('hwe-t', 'hwe-v')
        osystem = make_usable_osystem(self, 'ubuntu', ['precise'])
    ...
    def test_POST_start_with_old_release_and_newer_min_hwe_kernel(self):
        osystem = make_usable_osystem(self, 'ubuntu', ['trusty'])
        distro_series = osystem['default_release']
        node = factory.make_Node(
            owner=self.logged_in_user, mac=True, power_type='ether_wake',
