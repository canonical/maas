...
def validate_power_params_fips(
    ...
    # 'power_verify_ssl': "y" → verify the certificate; "n" (default) → skip.
    # The driver constant is SSL_INSECURE_YES="y" (backward naming:
    # "insecure=yes" actually enables verification).  Reject skip.
    if power_type in {"webhook", "proxmox", "hmcz"}:
        if power_parameters.get("power_verify_ssl", "n") != "y":
            raise ValidationError(
