...
Selection = namedtuple('Selection', ['boot_source', 'os', 'release'])
Values = namedtuple('Values', ['arches', 'subarches', 'labels'])


class Migration(SchemaMigration):

    def forwards(self, orm):
        # Coalesce all the arch/subarch/label fields for duplicated
        # records across release/os/boot_source.  Do this by using the
        # Selection as the key in a dict containing Values.
        boot_sources = dict()
        for bss in orm['maasserver.bootsourceselection'].objects.all():
            selection = Selection(
                boot_source=bss.boot_source,
                os=bss.os,
                release=bss.release,
                )
            values = Values(
                arches=set(bss.arches),
                subarches=set(bss.subarches),
                labels=set(bss.labels),
                )
            if selection not in boot_sources:
                # New boot_source/os/release combo.
                boot_sources[selection] = values
            else:
                # Merge with existing.
                arches_set = boot_sources[selection].arches
                subarches_set = boot_sources[selection].subarches
                labels_set = boot_sources[selection].labels

                boot_sources[selection] = Values(
                    arches=arches_set.union(values.arches),
                    subarches=subarches_set.union(values.subarches),
                    labels=labels_set.union(values.labels),
                    )

        # Delete all the existing rows in the table.  Nothing currently
        # has BootSourceSelection as a foreign key, so Django's default
        # behaviour of ON DELETE CASCADE should not delete anything
        # else here.
        orm['maasserver.bootsourceselection'].objects.all().delete()

        now = datetime.datetime.utcnow()
        # Add the coalesced records.
        for selection, values in boot_sources.items():
            bss = orm['maasserver.BootSourceSelection'](
                created=now,
                updated=now,
                boot_source=selection.boot_source,
                os=selection.os,
                release=selection.release,
                arches=list(values.arches),
                subarches=list(values.subarches),
                labels=list(values.labels),
                )
            bss.save()

        # This happens in the next migration as you cannot do data and
        # schema in the same one.
        # Adding unique constraint on 'BootSourceSelection', fields ['release', 'boot_source', 'os']
        #db.create_unique(u'maasserver_bootsourceselection', ['release', 'boot_source_id', 'os'])


    def backwards(self, orm):
        # Removing unique constraint on 'BootSourceSelection', fields ['release', 'boot_source', 'os']
        #db.delete_unique(u'maasserver_bootsourceselection', ['release', 'boot_source_id', 'os'])
        pass
    ...
    models = {
        ...
        'maasserver.filestorage': {
            ...
            'filename': ('django.db.models.fields.CharField', [], {'max_length': '255'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'key': ('django.db.models.fields.CharField', [], {'default': "u'0121a28a-6899-11e4-b353-0026c71eea0e'", 'unique': 'True', 'max_length': '36'}),
        ...
        'maasserver.node': {
            ...
            'pxe_mac': ('django.db.models.fields.related.ForeignKey', [], {'default': 'None', 'related_name': "u'+'", 'null': 'True', 'blank': 'True', 'to': "orm['maasserver.MACAddress']"}),
            ...
            'system_id': ('django.db.models.fields.CharField', [], {'default': "u'node-012616a8-6899-11e4-b353-0026c71eea0e'", 'unique': 'True', 'max_length': '41'}),
        ...
        'piston.token': {
            ...
            'key': ('django.db.models.fields.CharField', [], {'max_length': '18'}),
            'secret': ('django.db.models.fields.CharField', [], {'max_length': '32'}),
            'timestamp': ('django.db.models.fields.IntegerField', [], {'default': '1415596707L'}),
