...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # Migration the MACAddress data into the Interface table.
        for interface in orm['maasserver.Interface'].objects.all():
            mac = interface.mac
            if mac is not None:
                interface.node = mac.node
                interface.mac_address = mac.mac_address
                interface.mac = None
                interface.save()

        # Link the Interface to the boot_interface for the pxe_mac.
        for node in orm['maasserver.Node'].objects.all():
            if node.pxe_mac is not None:
                interface = orm['maasserver.Interface'].objects.filter(
                    type="physical", node=node,
                    mac_address=node.pxe_mac.mac_address).first()
                if interface is not None:
                    node.boot_interface = interface
                    node.save(update_fields=['boot_interface'])

    def backwards(self, orm):
        # Create the MACAddress from the Interface table.
        for interface in orm['maasserver.Interface'].objects.filter(
                node__isnull=False, mac_address__isnull=False):
            now = datetime.datetime.now()
            mac = orm['maasserver.MACAddress'].objects.create(
                node=interface.node, mac_address=interface.mac_address,
                created=now, updated=now)
            interface.mac = mac
            interface.save()

        # Link the MACAddress to the pxe_mac from the boot_interface.
        for node in orm['maasserver.Node'].objects.all():
            if node.boot_interface is not None:
                mac = orm['maasserver.MACAddress'].objects.filter(
                    node=node,
                    mac_address=node.boot_interface.mac_address).first()
                if mac is not None:
                    node.pxe_mac = mac
                    node.save(update_fields=['pxe_mac'])
    ...
    models = {
        ...
        'maasserver.filestorage': {
            ...
            'filename': ('django.db.models.fields.CharField', [], {'max_length': '255'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'key': ('django.db.models.fields.CharField', [], {'default': "u'104b519c-4acb-11e5-b726-bcee7b78dc5b'", 'unique': 'True', 'max_length': '36'}),
        ...
        'maasserver.node': {
            ...
            'system_id': ('django.db.models.fields.CharField', [], {'default': "u'node-1048b216-4acb-11e5-b726-bcee7b78dc5b'", 'unique': 'True', 'max_length': '41'}),
        ...
        'piston.token': {
            ...
            'key': ('django.db.models.fields.CharField', [], {'max_length': '18'}),
            'secret': ('django.db.models.fields.CharField', [], {'max_length': '32'}),
            'timestamp': ('django.db.models.fields.IntegerField', [], {'default': '1440467171L'}),
