...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.delete_unique(u'maasserver_filesystem', ['partition_id'])

        # Removing unique constraint on 'Filesystem', fields ['block_device']
        db.delete_unique(u'maasserver_filesystem', ['block_device_id'])

        # Removing unique constraint on 'Filesystem', fields ['uuid']
        db.delete_unique(u'maasserver_filesystem', ['uuid'])

        # Adding field 'Filesystem.acquired'
        db.add_column(u'maasserver_filesystem', 'acquired',
                      self.gf('django.db.models.fields.BooleanField')(default=False),
                      keep_default=False)

        # Adding unique constraint on 'Filesystem', fields ['acquired', 'partition']
        db.create_unique(u'maasserver_filesystem', ['acquired', 'partition_id'])

        # Adding unique constraint on 'Filesystem', fields ['block_device', 'acquired']
        db.create_unique(u'maasserver_filesystem', ['block_device_id', 'acquired'])


    def backwards(self, orm):
        # Removing unique constraint on 'Filesystem', fields ['block_device', 'acquired']
        db.delete_unique(u'maasserver_filesystem', ['block_device_id', 'acquired'])

        # Removing unique constraint on 'Filesystem', fields ['acquired', 'partition']
        db.delete_unique(u'maasserver_filesystem', ['acquired', 'partition_id'])

        # Deleting field 'Filesystem.acquired'
        db.delete_column(u'maasserver_filesystem', 'acquired')

        # Adding unique constraint on 'Filesystem', fields ['uuid']
        db.create_unique(u'maasserver_filesystem', ['uuid'])

        # Adding unique constraint on 'Filesystem', fields ['block_device']
        db.create_unique(u'maasserver_filesystem', ['block_device_id'])

        # Adding unique constraint on 'Filesystem', fields ['partition']
        db.create_unique(u'maasserver_filesystem', ['partition_id'])
