...
# Make sure the final state (indicative of when a node can be used by a
# user) of a node is status #6.

# Hardcode the values of the statuses instead of using the enum
# module to avoid breakage if the meaning of the values changes
# later.
new_allocated_status = 10
new_deployed_status = 6

intermediate_status = 1000

old_allocated_status = 6
old_deployed_status = 10


class Migration(DataMigration):

    def forwards(self, orm):
        deployed_nodes = orm['maasserver.Node'].objects.filter(status=old_deployed_status)
        deployed_nodes.update(status=intermediate_status)

        allocated_nodes = orm['maasserver.Node'].objects.filter(status=old_allocated_status)
        allocated_nodes.update(status=new_allocated_status)

        deployed_nodes = orm['maasserver.Node'].objects.filter(status=intermediate_status)
        deployed_nodes.update(status=new_deployed_status)
    ...
    models = {
        ...
        u'maasserver.filestorage': {
            ...
            'filename': ('django.db.models.fields.CharField', [], {'max_length': '255'}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'key': ('django.db.models.fields.CharField', [], {'default': "u'120c95d0-3a83-11e4-941e-3c970e0e56dc'", 'unique': 'True', 'max_length': '36'}),
        ...
        u'maasserver.node': {
            ...
            'system_id': ('django.db.models.fields.CharField', [], {'default': "u'node-1209873c-3a83-11e4-941e-3c970e0e56dc'", 'unique': 'True', 'max_length': '41'}),
        ...
        u'piston.token': {
            ...
            'key': ('django.db.models.fields.CharField', [], {'max_length': '18'}),
            'secret': ('django.db.models.fields.CharField', [], {'max_length': '32'}),
            'timestamp': ('django.db.models.fields.IntegerField', [], {'default': '1410529534L'}),
