...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.add_column(u'maasserver_node', 'boot_type',
                      self.gf('django.db.models.fields.CharField')(default=u'fastpath', max_length=20),
                      keep_default=False)

        # Set the boot_type based on the tag set on Node.
        for node in orm['maasserver.Node'].objects.all():
            if node.tags.filter(name="use-fastpath-installer").exists():
                node.boot_type = NODE_BOOT.FASTPATH
            else:
                node.boot_type = NODE_BOOT.DEBIAN
            node.save()

        # Delete the "use-fastpath-installer" tag, as it is no longer used
        orm['maasserver.Tag'].objects.filter(
            name="use-fastpath-installer").delete()


    def backwards(self, orm):
        # Create the "use-fastpath-installer" tag.
        current_time = now()
        fpi_tag = orm['maasserver.Tag'].objects.get_or_create(
