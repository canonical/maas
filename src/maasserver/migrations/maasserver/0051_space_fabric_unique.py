    ...
    for instance in duplicates:
        ...
        if cls.objects.filter(name=name).exists():
            ...
            while retries > 0 and cls.objects.filter(name=name).exists():
                name = '%s-%d-%d' % (
                    instance.name, instance.id, randint(1, 100000))
                retries -= 1
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('maasserver', '0050_modify_external_dhcp_on_vlan'),
    ]
    ...
    operations = [
        ...
        migrations.AlterField(
            model_name='space',
            name='name',
            field=models.CharField(null=True, blank=True, unique=True, validators=[maasserver.models.space.validate_space_name], max_length=256),
        ),
        migrations.AlterField(
            model_name='fabric',
            name='name',
            field=models.CharField(null=True, blank=True, unique=True, validators=[maasserver.models.fabric.validate_fabric_name], max_length=256),
        ),
    ]
