...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # MAAS accepts a list of DNS servers. In the CLI and UI this list is
        # provided as a space seperated list which is then converted into a
        # Python list and finally a SQL array. MAAS < 1.8 did not check for
        # comma seperated lists and added them as one entry to the database.
        for subnet in orm['maasserver.Subnet'].objects.all():
            new_dns_servers = []
            for dns_server in subnet.dns_servers:
                if ',' in dns_server:
                    for i in dns_server.split(','):
                        if i:
                            new_dns_servers.append(i)
                else:
                    new_dns_servers.append(dns_server)
            subnet.dns_servers = new_dns_servers
            subnet.save()

        for config in orm['maasserver.Config'].objects.all():
            if (config.name == "upstream_dns" and
                config.value is not None and ',' in config.value):
                # From IPListFormField
                separators = re.compile('[,\s]+')
                ips = re.split(separators, config.value)
                ips = [ip.strip() for ip in ips if ip != '']
                config.value = ' '.join(ips)
                config.save()
            

    def backwards(self, orm):
        # No migration needed as this just fixes misformatted data
        return
    ...
    models = {
        ...
        u'maasserver.filestorage': {
            ...
            'filename': ('django.db.models.fields.CharField', [], {'max_length': '255'}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'key': ('django.db.models.fields.CharField', [], {'default': "u'da7bf4c7-97b5-11e5-b89d-525400cf186e'", 'unique': 'True', 'max_length': '36'}),
        ...
        u'maasserver.node': {
            ...
            'system_id': ('django.db.models.fields.CharField', [], {'default': "u'node-da7bf4c6-97b5-11e5-b89d-525400cf186e'", 'unique': 'True', 'max_length': '41'}),
        ...
        u'piston.token': {
            ...
            'key': ('django.db.models.fields.CharField', [], {'max_length': '18'}),
            'secret': ('django.db.models.fields.CharField', [], {'max_length': '32'}),
            'timestamp': ('django.db.models.fields.IntegerField', [], {'default': '1448924301'}),
