...
class BaseFilesystemGroupManager(Manager):
    ...
    def get_available_name_for(self, filesystem_group):
        ...
        node = filesystem_group.get_node()
        idx = -1
        # Find existing groups with matching prefix to determine next index.
        # If no node yet: search globally. RAID types share 'md' prefix, so check all together.
        if node is not None:
            queryset = self.filter_by_node(node)
        else:
            if filesystem_group.group_type in FILESYSTEM_GROUP_RAID_TYPES:
                queryset = self.filter(group_type__in=FILESYSTEM_GROUP_RAID_TYPES)
            else:
...
class FilesystemGroup(CleanSave, TimestampedModel):
    ...
    def save(self, *args, **kwargs):
        ...
        # if needed. Generate a unique name if one wasn't explicitly set.
