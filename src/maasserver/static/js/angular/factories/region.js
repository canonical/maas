...
angular.module('MAAS').factory(
    'RegionConnection', [
        '$q', '$rootScope', '$timeout', '$window', '$cookies', 'LogService',
        function() { return new RegionConnection(...arguments); }
    ]
);
    ...
    class RegionConnection {
        ...
        ensureConnection() {
            ...
            if(this.state === REGION_STATE.UP) {
                ...
                if(missedPings > 0) {
                    this.log.debug(
                        "Still waiting for ping replies: ",
                        this.pingsInFlight);
                }
                if(missedPings >= this.maxMissedPings) {
                    ...
                    this.retry();
                        ...
                        impatienceFactor >= this.maxPatience) {
                    this.log.warn(
                        "Closed connection: impatience factor exceeded.");
                    this.retry();
                }
                else {
                    this.ping();
            ...
            if(angular.isDefined(defer)) {
                if(msg.rtype === RESPONSE_TYPE.SUCCESS) {
                    ...
                    if(angular.isObject(remembered_request)) {
                        this.$rootScope.$apply(defer.reject({
                            "error": msg.error,
