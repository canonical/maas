        ...
        if from_db:
            ...
            if not created:
                ...
                create_audit_event(
                    ...
                    description=f"Created boot source {url}",
                    action=CREATED,
                    id=source.id,
                )
                ...
                # we have to iterate on all the boot sources because `QuerySet.delete()`
        ...
        if resource.rtype == BOOT_RESOURCE_TYPE.SYNCED:
            ...
            for selection in BootSourceSelection.objects.all():
                if image_passes_filter(
                    ...
                    # This will also delete all the resources associated
                    selection.delete()
                    create_audit_event(
                        event_type=EVENT_TYPES.BOOT_SOURCE_SELECTION,
                        endpoint=ENDPOINT.UI,
