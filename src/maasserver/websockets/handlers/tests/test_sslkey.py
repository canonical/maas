...
class TestSSLKeyHandler(MAASServerTestCase):
    ...
    def test_list(self):
        ...
        expected_sslkeys = [
            self.dehydrate_sslkey(sslkey)
            for sslkey in SSLKey.objects.all()
            ]
    ...
    def test_create(self):
        ...
        new_sslkey = handler.create({
            'key': key_string,
        })
        self.assertThat(new_sslkey, ContainsDict({
            "user": Equals(user.id),
    ...
    def test_delete(self):
        ...
        self.assertIsNone(
            get_one(SSLKey.objects.filter(id=sslkey.id)))
