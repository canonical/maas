...
def list_machines_one_query(conn, admin, limit=None):
    ...
    # XXX Investigate if there's a more performant way of getting spaces
    #     and fabrics. The current approach with CTEs made the listing
    #     go from around 50ms to 70ms.
    vlans_cte = (
        select(
            Vlan.c.id,
            Machine.c.id.label("machine_id"),
    ...
    # XXX: Investigate whether we can restructure the interfaces CTEs
    #      to make them more re-usable, and perform better.
    extra_macs_cte = (
        select(
            Machine.c.id,
    ...
    status_message_cte = (
        ...
        .select_from(Event)
        .distinct(Machine.c.id)
        .join(Machine, Event.c.node_id == Machine.c.id)
        .join(EventType, EventType.c.id == Event.c.type_id)
        .order_by(Machine.c.id, Event.c.created.desc(), Event.c.id.desc())
        .where(
    ...
    stmt = (
        select(
            *[getattr(Machine.c, attr) for attr in PLAIN_LIST_ATTRIBUTES],
            ...
            BMC.c.power_type,
            status_message_cte.c.status_message,
        ...
        .join(status_message_cte, status_message_cte.c.id == Machine.c.id)
